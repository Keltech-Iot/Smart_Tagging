    ...
    <header>
        ...
        @foreach($aerialNet as $row)
            ...
            <tr>
                ...
                <td><a href="{{route('digi_map_aerial', ['lat'=>$row->lat, 'lng'=>$row->long, 'id'=>$row->aerial_id])}}"> {{$row->location}}</a> </td>
            </tr>
        </table>
        <a href="digi_edit/{{$row->aerial_id}}" class="button"><span>Edit</span></a>
        @endforeach
</article>
</div>
</div>
